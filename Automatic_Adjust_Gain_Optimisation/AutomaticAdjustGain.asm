...
;*** Nombre de cycle de loop: 8 cycles * 128 = 1024 cycles
_FindGain:
		MVK		.S1		31, A10			; Maximum de décalage à gauche sur 32 bits
		MV		.L2		B4, B0			; nombre d'échantillons N --> registre A1
loop:
		LDH		.D1		*A4++, A11		; Lecture d'un échantillons Xn dans XnBuffer
  		SUB		.S2		B0, 1, B0		; Décrementation du compteur d'itération
  [B0]	B		.S1		loop			; Boucle pour faire 128 itérations
		NOP		2
		NORM	.L1		A11, A12		; Redondance du bit de signe de l'échantillon Xn --> A12
		CMPLT	.L1		A12, A10, A2	; Si (Redondance du bit de signe de l'échantillon Xn) < Min Redondance  --> A2 = 1
  [A2]	MV		.S1		A12, A10		; Si oui (Redondance du bit de signe de l'échantillon Xn) --> Min Redondance

  		SUB		.S1		A10, 16 , A4	; Les échantillons sont en 16 bits signés (short), or l'instruction NORM détermine  										; la redondance sur 32 bits (16 bits de plus)
